...
import { convertToRgb } from './colorCalculations.js'
...
export function generatePDF(typography, colors) {
  ...
  pdf.text(typography.general.fontSize + 'px', 50, 85)
  pdf.text('Line Height:', 10, 90)
  pdf.text(typography.general.lineHeight + 'px', 50, 90)
  pdf.text('Text Width:', 10, 95)
  pdf.text(typography.general.textWidth + 'px', 50, 95)
  pdf.text('Paragraph Spacing:', 10, 100)
  pdf.text(typography.general.textSpacing + 'px', 50, 100)

  pdf.setFontSize(14)
  pdf.text('Headline 1', 10, 120)
  pdf.setFontSize(10)
  pdf.text('Size', 10, 125)
  pdf.text(typography.headline1.size + 'px', 50, 125)
  pdf.text('Spacing Top', 10, 130)
  pdf.text(typography.headline1.spacingTop + 'px', 50, 130)
  pdf.text('Spacing Bottom', 10, 135)
  pdf.text(typography.headline1.spacingBottom + 'px', 50, 135)

  pdf.setFontSize(14)
  pdf.text('Headline 2', 70, 120)
  pdf.setFontSize(10)
  pdf.text('Size', 70, 125)
  pdf.text(typography.headline2.size + 'px', 110, 125)
  pdf.text('Spacing Top', 70, 130)
  pdf.text(typography.headline2.spacingTop + 'px', 110, 130)
  pdf.text('Spacing Bottom', 70, 135)
  pdf.text(typography.headline2.spacingBottom + 'px', 110, 135)

  pdf.setFontSize(14)
  pdf.text('Headline 3', 130, 120)
  pdf.setFontSize(10)
  pdf.text('Size', 130, 125)
  pdf.text(typography.headline3.size + 'px', 170, 125)
  pdf.text('Spacing Top', 130, 130)
  pdf.text(typography.headline3.spacingTop + 'px', 170, 130)
  pdf.text('Spacing Bottom', 130, 135)
  pdf.text(typography.headline3.spacingBottom + 'px', 170, 135)

  pdf.setFontSize(14)
  pdf.text('Colors', 10, 155)
  pdf.setFontSize(10)
  pdf.text('Foreground:', 10, 160)
  pdf.text(typography.colors.foreground, 50, 160)
  pdf.text('Background', 10, 165)
  pdf.text(typography.colors.background, 50, 165)
  ...
  let xVal = 20
  for (var i = 0; i < colors.baseColors.length; i++) {
    let currColor = colors.baseColors[i]
    let rgbColor = convertToRgb(currColor)
    pdf.setFillColor(rgbColor.r, rgbColor.g, rgbColor.b)
    pdf.circle(xVal, 210, 10, 'F')
    pdf.text(currColor, xVal - 5, 225)
    xVal += 30
  }

  for (var j = 0; j < colors.contrast.colors.length; j++) {
    let currColor = colors.contrast.colors[j]
    let rgbColor = convertToRgb(currColor)
    pdf.setFillColor(rgbColor.r, rgbColor.g, rgbColor.b)
    pdf.circle(xVal, 210, 10, 'F')
    pdf.text(currColor, xVal - 5, 225)
